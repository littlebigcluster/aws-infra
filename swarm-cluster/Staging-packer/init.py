...
# To be sure 1rst swarm init not at same time
sleep(randint(1,60))
...
managers_running = describe_nodes([{'Name': 'tag:Name', 'Values': ['SwarmManager-staging']}, {'Name': 'instance-state-name', 'Values': ['running']}, {'Name': 'tag:Init', 'Values': ['true']}])
managers_replaced = describe_nodes([{'Name': 'tag:Name', 'Values': ['SwarmManager-staging']}, {'Name': 'instance-state-name', 'Values': ['shutting-down', 'stopped', 'terminated', 'stopping']}, {'Name': 'tag:Init', 'Values': ['true']}])
workers_replaced = describe_nodes([{'Name': 'tag:Name', 'Values': ['SwarmWorker-staging']}, {'Name': 'instance-state-name', 'Values': ['shutting-down', 'stopped', 'terminated', 'stopping']}])
...
if instances_count > 0 or role == 'worker':
    ...
    # token = run(['"docker", "-H {}:2375".format(manager), "swarm", "join-token", "-q", role], stdout=PIPE') # get the token
    token = run(["docker", "-H {}".format(manager), "swarm", "join-token", "-q", role], stdout=PIPE) # get the token
